...
export function PreviewCanvas({ code, isForging, deviceWidthPx }: PreviewCanvasProps) {
  return (
    <div className="relative h-full min-h-[20rem] overflow-hidden rounded-md bg-muted ring-1 ring-rule-structural sm:min-h-[28rem]">
      {deviceWidthPx && (
        <div className="absolute left-0 right-0 top-0 z-10 flex justify-center py-1">
          <span className="pf-kicker text-ember-strong">{deviceWidthPx} × auto</span>
        </div>
      )}
      ...
      {code ? (
        ...
        <div className="flex h-full items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">The rendered result appears here</p>
          </div>
        </div>
      )}
    </div>
